<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800">Nueva Oferta</h2>
                ...
                <form action="{{ route('empresa.ofertas.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="titulo" name="titulo" type="text"
                            class="mt-1 block w-full" value="{{ old('titulo') }}" required />
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="requisitos" name="requisitos" rows="3"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-200">{{ old('requisitos') }}</textarea>
                    </div>

                    {{-- Especialidades --}}
                    <div class="mb-4">
                        <x-input-label value="Especialidades requeridas *" />
                        <p class="text-xs text-gray-500 mb-2">Seleccioná una o más. Marcá cuál es la principal.</p>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="especialidad_principal" id="especialidad_principal"
                            ...
                            @foreach($especialidades as $esp)
                                <option value="{{ $esp->id }}"
                                    {{ old('especialidad_principal') == $esp->id ? 'selected' : '' }}>
                                    {{ $esp->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <x-input-error :messages="$errors->get('tipo_contrato')" class="mt-1" />
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <x-input-label for="experiencia_requerida" value="Años de experiencia requeridos" />
                            <x-text-input id="experiencia_requerida" name="experiencia_requerida" type="number"
                                class="mt-1 block w-full" value="{{ old('experiencia_requerida') }}" min="0" />
                        </div>
                        <div>
                            ...
                            <x-text-input id="fecha_vencimiento" name="fecha_vencimiento" type="date"
                                class="mt-1 block w-full" value="{{ old('fecha_vencimiento') }}" />
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select name="estado" id="estado"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach(['Activa','Borrador','Pausada'] as $est)
                                <option value="{{ $est }}" {{ old('estado', 'Activa') == $est ? 'selected' : '' }}>
                                    {{ $est }}
                                </option>
                            @endforeach
                        </select>
