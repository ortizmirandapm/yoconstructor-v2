...
<div class="px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Gesti&oacute;n de Administradores</h1>
            <p class="text-sm text-gray-500 mt-1">Administrar cuentas de administrador del sistema</p>
        </div>
    ...
    @endif
    @if(session('error'))
        <div id="toast-error" role="alert" aria-live="assertive" aria-atomic="true" class="fixed top-6 right-6 z-[9999] flex items-center gap-3 bg-red-50 border border-red-200 text-red-800 px-5 py-4 rounded-xl shadow-xl max-w-sm transition-all duration-500">
        ...
        <script>setTimeout(function(){var t=document.getElementById("toast-error");if(t)t.remove();},4000);</script>
    @endif

    {{-- FILTERS --}}
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 sm:p-6 mb-6">
        <form method="GET" action="{{ route('admin.administradores.index') }}" class="flex flex-col lg:flex-row gap-4 items-end">
    ...
    <div class="flex items-center gap-2 flex-wrap mb-4">
        <a href="{{ route('admin.administradores.index', array_merge(request()->except('estado'), ['estado' => ''])) }}"
        ...
        <a href="{{ route('admin.administradores.index', array_merge(request()->except('estado'), ['estado' => 'activos'])) }}"
        ...
        <a href="{{ route('admin.administradores.index', array_merge(request()->except('estado'), ['estado' => 'inactivos'])) }}"
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Administrador</th>
                        <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Estado</th>
                        <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider hidden sm:table-cell">Registrado</th>
                        <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Acciones</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse($administradores as $admin)
                        @php $esActivo = $admin->estado; @endphp
                        <tr class="hover:bg-gray-50 transition {{ !$esActivo ? 'opacity-60 bg-gray-50/50' : '' }}">
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 bg-red-100 rounded-xl flex items-center justify-center text-red-700 text-sm font-bold flex-shrink-0 border border-red-200">
                                        {{ strtoupper(substr($admin->name ?: 'A', 0, 1)) }}
                                    </div>
                                    <div class="min-w-0">
                                        <p class="font-semibold text-gray-800 truncate">{{ $admin->name }}</p>
                            ...
                            <td class="px-4 py-4 text-center">
                                <div class="relative inline-block text-left">
                                    <button type="button"
                                        onclick="toggleDropdown(event, 'dropdown-{{ $admin->id }}')"
                                        aria-haspopup="true"
                                        aria-expanded="false"
                                    ...
                                    <div id="dropdown-{{ $admin->id }}"
                                        ...
                                        <div class="py-1">
                                            ...
                                            @if($admin->id !== auth()->id())
                                                <button type="button"
                                                    onclick="abrirModalToggle({{ $admin->id }}, '{{ addslashes($admin->name) }}', {{ $esActivo ? 'true' : 'false' }})"
                                        ...
                                        @if($admin->id !== auth()->id())
                                            <div class="py-1">
                                                <button type="button"
                                                    onclick="abrirModalEliminar({{ $admin->id }}, '{{ addslashes($admin->name) }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-5 py-12 text-center">
                                <p class="text-gray-400 text-sm">No hay administradores</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div id="modalCrear" role="dialog" aria-modal="true" aria-labelledby="modal-crear-titulo" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <form method="POST" action="{{ route('admin.administradores.store') }}" class="overflow-y-auto flex-1">
            @csrf
            <div class="p-6 space-y-4">
                <div>
                    <label for="crear-name" class="block text-xs font-semibold text-gray-600 mb-1.5">Nombre completo <span class="text-red-500">*</span></label>
                    <input type="text" name="name" id="crear-name" required placeholder="Nombre del administrador"
                ...
                <div>
                    ...
                    <input type="email" name="email" id="crear-email" required placeholder="admin@ejemplo.com"
                        class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                        aria-required="true">
                </div>
...
<div id="modalEditar" role="dialog" aria-modal="true" aria-labelledby="modal-editar-titulo" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <form method="POST" action="" id="form-editar" class="overflow-y-auto flex-1">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label for="editar-name" class="block text-xs font-semibold text-gray-600 mb-1.5">Nombre completo <span class="text-red-500">*</span></label>
                    <input type="text" name="name" id="editar-name" required
                        class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                        aria-required="true">
                </div>
                <div>
                    <label for="editar-email" class="block text-xs font-semibold text-gray-600 mb-1.5">Email <span class="text-red-500">*</span></label>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="relative">
                            ...
                            <input type="password" name="password" id="editar-password" placeholder="Dejar vac&iacute;o"
                                class="w-full px-3 py-2.5 pr-10 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
                    ...
                    <p class="text-xs text-gray-400 mt-2">Dej&aacute; los campos vac&iacute;os si no quer&eacute;s modificar la contrase&ntilde;a.</p>
                </div>
            </div>
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-3 flex-shrink-0">
...
<div id="modalEliminar" role="dialog" aria-modal="true" aria-labelledby="eliminar-titulo" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div class="px-6 py-5 border-b border-gray-200 flex items-center gap-3">
            ...
            <div>
                <h3 id="eliminar-titulo" class="font-bold text-gray-900">Eliminar administrador</h3>
            ...
            <div class="px-6 py-5">
                <div class="p-4 bg-red-50 border border-red-200 rounded-xl">
                    ...
                    <ul class="text-xs text-red-600 space-y-0.5 ml-3 list-disc">
                        <li>Se eliminar&aacute; la cuenta de administrador</li>
                        <li>No se podr&aacute; recuperar</li>
                    </ul>
                </div>
            </div>
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-2xl flex justify-end gap-3">
...
<script>
    ...
    function abrirModalEditar(id) {
        document.getElementById('form-editar').action = '{{ url("admin/administradores") }}/' + id;
        fetch('{{ url("admin/administradores") }}/' + id + '/edit')
            .then(function(r) { return r.json(); })
            .then(function(data) {
                document.getElementById('editar-name').value = data.name || '';
            ...
            .catch(function() { alert('Error al cargar datos del administrador'); });
    }
    ...
    function abrirModalToggle(id, nombre, esActivo) {
        document.getElementById('form-toggle').action = '{{ url("admin/administradores") }}/' + id + '/estado';
        ...
        document.getElementById('toggle-mensaje').innerHTML   = esActivo
            ? '&iquest;Est&aacute;s seguro de que quer&eacute;s <strong>desactivar</strong> a este administrador?'
            : '&iquest;Est&aacute;s seguro de que quer&eacute;s <strong>activar</strong> a este administrador?';
    ...
    function abrirModalEliminar(id, nombre) {
        document.getElementById('form-eliminar').action = '{{ url("admin/administradores") }}/' + id;
