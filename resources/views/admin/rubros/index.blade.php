...
<div class="px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Gesti&oacute;n de Rubros</h1>
            <p class="text-sm text-gray-500 mt-1">Administrar rubros de la plataforma</p>
        </div>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 sm:p-6 mb-6">
        <form method="GET" action="{{ route('admin.rubros.index') }}" class="flex flex-col lg:flex-row gap-4 items-end">
    ...
    <div class="flex items-center gap-2 flex-wrap mb-4">
        <a href="{{ route('admin.rubros.index', array_merge(request()->except('estado'), ['estado' => ''])) }}"
        ...
        <a href="{{ route('admin.rubros.index', array_merge(request()->except('estado'), ['estado' => 'activos'])) }}"
        ...
        <a href="{{ route('admin.rubros.index', array_merge(request()->except('estado'), ['estado' => 'inactivos'])) }}"
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Rubro</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse($rubros as $rubro)
                        <tr class="hover:bg-gray-50 transition {{ !$rubro->estado ? 'opacity-60 bg-gray-50/50' : '' }}">
                            <td class="px-5 py-4">
                                <div class="min-w-0">
                                    <p class="font-semibold text-gray-800">{{ $rubro->nombre }}</p>
                                    @if($rubro->descripcion)
                                        <p class="text-xs text-gray-400 truncate max-w-[300px]">{{ $rubro->descripcion }}</p>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-4 text-center hidden sm:table-cell text-sm text-gray-400">
                                {{ $rubro->created_at->format('d/m/Y') }}
                            </td>
                            <td class="px-4 py-4 text-center">
                                <div class="relative inline-block text-left">
                                    <button type="button"
                                        onclick="toggleDropdown(event, 'dropdown-{{ $rubro->id }}')"
                                    ...
                                    <div id="dropdown-{{ $rubro->id }}"
                                        ...
                                        <div class="py-1">
                                            <button type="button"
                                                onclick="abrirModalEditar({{ $rubro->id }}, '{{ addslashes($rubro->nombre) }}', '{{ addslashes($rubro->descripcion ?? '') }}')"
                                                class="flex w-full px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 transition text-left"
                                                role="menuitem">
                                                Editar
                                            </button>
                                            <button type="button"
                                                onclick="abrirModalToggle({{ $rubro->id }}, '{{ addslashes($rubro->nombre) }}', {{ $rubro->estado ? 'true' : 'false' }})"
                                                class="flex w-full px-4 py-2 text-sm text-left transition {{ $rubro->estado ? 'text-orange-700 hover:bg-orange-50' : 'text-green-700 hover:bg-green-50' }}"
                                                role="menuitem">
                                                {{ $rubro->estado ? 'Desactivar' : 'Activar' }}
                                            </button>
                                        </div>
                                        <div class="py-1">
                                            <button type="button"
                                                onclick="abrirModalEliminar({{ $rubro->id }}, '{{ addslashes($rubro->nombre) }}', {{ $rubro->ofertas_count }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-12 text-center">
                                <p class="text-gray-400 text-sm">No hay rubros</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div id="modalCrear" role="dialog" aria-modal="true" aria-labelledby="modal-crear-titulo" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <form method="POST" action="{{ route('admin.rubros.store') }}" class="overflow-y-auto flex-1">
            @csrf
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="nombre" id="crear-nombre" required placeholder="Ej: Construcci&oacute;n"
                ...
                <div>
                    ...
                    <textarea name="descripcion" id="crear-descripcion" rows="3" placeholder="Descripci&oacute;n del rubro"
...
<div id="modalEliminar" role="dialog" aria-modal="true" aria-labelledby="eliminar-titulo" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div class="px-6 py-5 border-b border-gray-200 flex items-center gap-3">
            ...
            <div>
                <h3 id="eliminar-titulo" class="font-bold text-gray-900">Eliminar rubro</h3>
            ...
            <div class="px-6 py-5">
                <div class="p-4 bg-red-50 border border-red-200 rounded-xl">
                    ...
                    <ul class="text-xs text-red-600 space-y-0.5 ml-3 list-disc">
                        <li>Se eliminar&aacute; la referencia en las ofertas asociadas</li>
                        <li id="eliminar-ofertas-warning" class="hidden">Actualmente tiene <strong id="eliminar-ofertas-count"></strong> ofertas vinculadas</li>
                    </ul>
                </div>
            </div>
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-2xl flex justify-end gap-3">
...
<script>
    ...
    function abrirModalEditar(id, nombre, descripcion) {
        document.getElementById('form-editar').action = '{{ url("admin/rubros") }}/' + id;
    ...
    function abrirModalToggle(id, nombre, esActivo) {
        document.getElementById('form-toggle').action = '{{ url("admin/rubros") }}/' + id + '/estado';
        ...
        document.getElementById('toggle-titulo').textContent  = esActivo ? 'Desactivar rubro' : 'Activar rubro';
        document.getElementById('toggle-nombre').textContent  = nombre;
        document.getElementById('toggle-mensaje').innerHTML   = esActivo
            ? '&iquest;Est&aacute;s seguro de que quer&eacute;s <strong>desactivar</strong> este rubro?'
            : '&iquest;Est&aacute;s seguro de que quer&eacute;s <strong>activar</strong> este rubro?';
    ...
    function abrirModalEliminar(id, nombre, ofertasCount) {
        document.getElementById('form-eliminar').action = '{{ url("admin/rubros") }}/' + id;
        document.getElementById('eliminar-nombre').textContent = nombre;
        var warning = document.getElementById('eliminar-ofertas-warning');
        var count = document.getElementById('eliminar-ofertas-count');
        if (ofertasCount > 0) {
            warning.classList.remove('hidden');
            count.textContent = ofertasCount;
        } else {
