...
final class EmpresaDashboardController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $ofertasActivas = (clone $ofertasQuery)->where('estado', 'Activa')->count();
        $ofertasPausadas = (clone $ofertasQuery)->where('estado', 'Pausada')->count();
        $ofertasBorradores = (clone $ofertasQuery)->where('estado', 'Borrador')->count();

        // ── Postulaciones stats ──
        $postQuery = Postulacion::whereHas('oferta', fn($q) => $q->where('empresa_id', $empresa->id));
        $postTotal = (clone $postQuery)->count();
        $postPendientes = (clone $postQuery)->where('estado', 'Pendiente')->count();
        $postRevisadas = (clone $postQuery)->where('estado', 'Revisada')->count();
        $postEntrevistas = (clone $postQuery)->where('estado', 'Entrevista')->count();
        $postAceptadas = (clone $postQuery)->where('estado', 'Aceptada')->count();
        $postRechazadas = (clone $postQuery)->where('estado', 'Rechazada')->count();
        ...
        $topOfertas = Oferta::withCount('postulaciones')
            ->withCount(['postulaciones as pendientes_count' => fn($q) => $q->where('estado', 'Pendiente')])
